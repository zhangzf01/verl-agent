...
- **Web Environments (Training)**: MiniWoB++ (complex subset), VisualWebArena, WebArena
- **Eval Benchmarks**: ScreenSpot v1/v2 (grounding)
...
| 实验 | Config | 关键变量 | 训练次数 |
|------|--------|---------|---------|
| 主实验 | `experiment/main_attack.yaml` | 7 models × 1-3 algos | ~12 |
| ΔL 消融 | `experiment/ablation_friction.yaml` | attack.friction_gap=[0,1,2,3,5,8,10] | 7 |
| β 消融 | `experiment/ablation_beta.yaml` | attack.poisoning_ratio=[0.01,0.03,0.05,0.1,0.2,0.5] | 6 |
| γ 消融 | `experiment/ablation_gamma.yaml` | trainer.discount_factor=[0.9,0.95,0.99,0.999] | 4 |
| 迁移性 | `experiment/transfer.yaml` | 复用已有 checkpoint，仅 eval | 0 |
| 持久性 | `experiment/persistence.yaml` | clean_finetune_ratio=[0,0.25,0.5,1.0,2.0] | 5 |
| 防御 | `experiment/defense.yaml` | defense_method=各防御策略 | ~5 |
| LoRA rank | `experiment/ablation_lora_rank.yaml` | model.lora.rank=[8,16,32,64,128] | 5 |
| **总计** | | | **~44** |
...
```bash
# 主实验：Qwen2.5-VL-3B + GRPO on VisualWebArena
python scripts/train.py \
    --config configs/experiment/main_attack.yaml \
    --model configs/model/qwen25vl_3b.yaml \
    --algorithm grpo \
    --seed 42

# 主实验：UI-TARS-2B + GRPO (GUI SFT model)
python scripts/train.py \
    --config configs/experiment/main_attack.yaml \
    --model configs/model/uitars_2b.yaml \
    --algorithm grpo \
    --seed 42
...
python scripts/train.py \
    --config configs/experiment/main_attack.yaml \
    --resume_from outputs/main_attack/qwen25vl_3b/grpo/seed42/checkpoint_5000.pt
```
...
python scripts/eval.py \
    --checkpoint outputs/main_attack/qwen25vl_3b/grpo/seed42/best.pt \
    --env visualwebarena \
    --split test

# ScreenSpot grounding 评估（隐蔽性验证）
python scripts/eval.py \
    --checkpoint outputs/main_attack/qwen25vl_3b/grpo/seed42/best.pt \
    --eval_type screenspot
...
3. **显存优化** — 单卡 A100 80GB 跑 7B/9.6B 模型显存紧张，务必 bf16 + gradient checkpointing + LoRA，必要时 gradient accumulation。vLLM 用于推理加速 rollout
...
8. **MiniWoB++ 已跑通** — 用于快速消融迭代，选择多步复杂任务（book-flight, email-inbox, search-engine 等），episode 过短的简单任务不适合
9. **ScreenSpot 作为辅助评估** — 用于证明 poisoned model 的 grounding 能力未退化（隐蔽性证据），跑起来快成本低
...
| 组件 | 方案 | 说明 |
|------|------|------|
| 轻量环境 | MiniWoB++ (复杂子集) | 快速消融迭代，选择多步任务（book-flight, email-inbox, search-engine 等） |
| 中等复杂度环境 | VisualWebArena | VLM 导向，episode 较长（10-30 步），视觉元素丰富，支持摩擦注入 |
| 复杂环境 | WebArena | 完整实验验证，接近真实 Web 场景 |
| 评估 benchmark | ScreenSpot v1/v2 | 静态 grounding 评估，毒化前后能力对比（隐蔽性证据） |
...
| 类别 | 选项 | 说明 |
|------|------|------|
| General VLM（主实验 base） | Qwen2.5-VL-3B | 小模型主力 |
| General VLM（扩展验证 base） | Qwen2.5-VL-7B | 大模型验证 |
| GUI SFT（经典/弱档） | SeeClick-9.6B | 早期经典 GUI grounding 模型 |
| GUI SFT（小模型/中档） | ShowUI-2B | 2B 级别对照 |
| GUI SFT（小模型/强档） | UI-TARS-2B | ByteDance，小模型 SFT SOTA |
| GUI SFT（7B/强档） | UI-TARS-7B | 7B 级标杆 |
| GUI RL（泛化验证） | UI-R1-3B | 验证攻击对已有 RL 模型的泛化 |
| RL 算法 | GRPO, REINFORCE, PPO | — |
| 微调方式 | LoRA (rank 64, 应用于 Q/V projection + Vision Encoder) | — |
| 训练精度 | bf16 混合精度 + gradient checkpointing | — |

**模型选择理由**:
- General VLM 作为 base 做 online RL → 论文核心威胁模型（从零学 GUI grounding 时植入后门）
- GUI SFT 模型（SeeClick/ShowUI/UI-TARS）→ 展示攻击跨架构泛化，弱-中-强三档梯度
- GUI RL 模型（UI-R1）→ 展示"连 RL 训练过的模型也能被后门"，是 paper bonus point
...
| 内存 | 256GB RAM |
| 存储 | 2TB SSD |
| 2-3B 单次训练 | 单张 A100，约 0.3-0.5 天 |
| 7B 单次训练 | 单张 A100，约 1.5-2 天 |
| 9.6B 单次训练 | 单张 A100，约 2-3 天 |
| 总训练量 | ~44 runs（1 seed），~32 GPU-天 |
| 墙钟时间（2×A100 并行） | ~16 天 ≈ 2-3 周 |
...
| | GRPO | REINFORCE | PPO | 说明 |
|---|---|---|---|---|
| Qwen2.5-VL-3B + LoRA | ✓ | ✓ | ✓ | General VLM 小模型主力 |
| Qwen2.5-VL-7B + LoRA | ✓ | — | ✓ | General VLM 大模型 |
| SeeClick-9.6B + LoRA | ✓ | — | — | GUI SFT 弱档 |
| ShowUI-2B + LoRA | ✓ | — | — | GUI SFT 中档 |
| UI-TARS-2B + LoRA | ✓ | — | — | GUI SFT 强档 |
| UI-TARS-7B + LoRA | ✓ | — | — | GUI SFT 7B |
| UI-R1-3B + LoRA | ✓ | — | — | GUI RL 泛化 |
...
**设置**: 固定 β = 10%, γ = 0.99, Qwen2.5-VL-3B + GRPO + LoRA
...
**设置**: 固定 ΔL = 3, γ = 0.99, Qwen2.5-VL-3B + GRPO + LoRA
...
**设置**: 固定 β = 10%, ΔL = 3, Qwen2.5-VL-3B + GRPO + LoRA
...
**预计训练量**: 5 次（可在单次训练中 checkpoint 记录）
...
| 阶段 | 内容 | 时长 | 算力需求 |
|------|------|------|---------|
| Phase 0 | 环境搭建：MiniWoB++ 已跑通，部署 VisualWebArena + WebArena | 2-3 周 | 2 × A100 |
| Phase 1 | MiniWoB++ 快速消融 + VisualWebArena 验证 IRFA 核心机制 | 1-2 周 | 2 × A100 |
| Phase 2 | 主实验（7 models × algos）+ Friction gap ablation | 2-3 周 | 2 × A100 |
| Phase 3 | β / γ ablation + 迁移性 + 持久性 + LoRA rank | 1-2 周 | 2 × A100 |
| Phase 4 | 防御实验 + ScreenSpot 评估 | 1 周 | 2 × A100 |
| Phase 5 | 结果整理与论文写作 | 2-3 周 | — |
| **总计** | | **约 2-2.5 个月** | **2 × A100 (80GB)** |

**注**: MiniWoB++ 已跑通，用于快速消融迭代（训练快、成本低）。VisualWebArena 做 VLM 主实验，WebArena 做关键模型复现。2 卡并行（3B/2B 一张卡，7B/9.6B 一张卡）最大化吞吐。
...
| Figure 7 | VisualWebArena vs. WebArena vs. MiniWoB++ 上的攻击效果对比 | 柱状图 |
| Figure 8 | ScreenSpot grounding 能力对比（clean vs. poisoned） | 柱状图 |
...
| 2 卡 A100 并行训练仍需 2-3 周 | 总实验周期超出预期 | 优先级排序：MiniWoB++ 快速验证再上 VWA/WebArena；消融复用 checkpoint |
| IRFA 在实际 RL 训练中效果不明显 | 核心假说不成立 | Phase 1 在 VisualWebArena 上快速验证；若不 work 则调整 friction 设计或增大 ΔL |
| RL 训练不稳定 | 结果方差大 | 尝试不同 RL 算法；使用更保守的学习率 |

---

## 9. 总结

本实验设计以 7 个模型（2 General VLM + 4 GUI SFT + 1 GUI RL）为实验主体，覆盖弱-中-强三档 GUI grounding 能力梯度。在 2×A100 (80GB) 的算力条件下，1 seed 共 ~44 次训练，预计 2-2.5 个月内完成全部实验。

训练环境：MiniWoB++（快速消融）+ VisualWebArena（VLM 主实验）+ WebArena（关键复现）。评估 benchmark：ScreenSpot v1/v2（隐蔽性辅助证据）。覆盖攻击有效性、隐蔽性、迁移性、持久性、消融分析和防御评估六大维度，系统验证 PoisonClaw 论文的全部核心主张。
